...
const FinancialMappingModal = ({ isOpen, onClose, statement, onLock, onDelete }) => {
    ...
    const [lines, setLines] = useState(() => {
        ...
        // Mock Data for "Manual Pro-Forma" (User Request: Shows ~40% progress)
        if (statement.name && statement.name.includes('Pro-Forma')) {
            return [
                // Mapped
                { id: 1, raw: 'Projected Sales', amount: 2000000, status: 'auto', section: 'Income Statement', category: 'Revenue', master: 'Sales / Revenue' },
                { id: 2, raw: 'Cost of Services', amount: 800000, status: 'manual', section: 'Income Statement', category: 'COGS', master: 'Cost of Goods Sold (Labor)' },

                // Unmapped
                { id: 3, raw: 'Rent Override', amount: 60000, status: 'unmapped', section: 'Income Statement', category: '', master: '' },
                { id: 4, raw: 'New Hire - Sales', amount: 85000, status: 'unmapped', section: 'Income Statement', category: '', master: '' },
                { id: 5, raw: 'New Hire - Ops', amount: 75000, status: 'unmapped', section: 'Income Statement', category: '', master: '' },
                { id: 6, raw: 'Marketing Push', amount: 50000, status: 'unmapped', section: 'Income Statement', category: '', master: '' },
                { id: 7, raw: 'Software Licenses', amount: 12000, status: 'unmapped', section: 'Income Statement', category: '', master: '' }
            ];
        }
        return [
            // Income Statement Items
            { id: 1, raw: 'Gross Revenue - Sales', amount: 1250000, status: 'auto', section: 'Income Statement', category: 'Revenue', master: 'Sales / Revenue' },
            { id: 2, raw: 'Service Income', amount: 450000, status: 'auto', section: 'Income Statement', category: 'Revenue', master: 'Sales / Revenue' },
            // Removed some items for variety in Account view vs Loan view if needed, but keeping consistent for now
            { id: 3, raw: 'Returns & Allowances', amount: -25000, status: 'manual', section: 'Income Statement', category: 'Revenue', master: 'Sales / Revenue' },
            { id: 4, raw: 'Cost of Goods Sold', amount: 650000, status: 'auto', section: 'Income Statement', category: 'COGS', master: 'Cost of Goods Sold (Materials)' },
            ...
            { id: 7, raw: 'Officer Comp - JD', amount: 120000, status: 'manual', section: 'Income Statement', category: 'Operating Expense', master: 'Owner Compensation', isOwnerComp: true },
            { id: 8, raw: 'Depreciation Exp', amount: 35000, status: 'auto', section: 'Income Statement', category: 'Operating Expense', master: 'Depreciation / Amortization (Non-Cash)', isNonCash: true },
            ...
            { id: 10, raw: 'Advertising Expense', amount: 12500, status: 'unmapped', section: 'Income Statement', category: '', master: '' },
            ...
            { id: 15, raw: 'Bank Loan Principal', amount: 250000, status: 'manual', section: 'Balance Sheet', category: 'Liability', master: 'Long Term Debt (LTD)' },
            { id: 16, raw: 'Suspense Account', amount: 5000, status: 'unmapped', section: 'Balance Sheet', category: '', master: '' }
        ];
    });
    ...
    const periodData = useMemo(() => {
        ...
        // Parse dates carefully - statement.startDate might be "Jan 1, 2023" or similar string, or ISO
        // For simplicity assuming standard date string or YYYY-MM-DD
        const start = new Date(statement.startDate);
        const end = new Date(statement.endDate);
        // Fallback for custom parsing if needed, but Date() usually handles standard formats well enough for prototypes

        // Defensive Date Check
        if (isNaN(start.getTime()) || isNaN(end.getTime())) {
            // Try splitting manual range string if passed as such (e.g. "Jan 1, 2023 - Dec 31, 2023")
            if (statement.period && statement.period.includes('-')) {
                // return placeholder, this is just a prototype
            }
            return { months: 12, factor: 1, label: 'Date Error (Defaulting)' };
        }
    ...
    const handleLock = () => {
        ...
        if (onLock) onLock({ ...statement, status: 'Locked', progress: 100 });
        onClose();
    };
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 shadow-sm z-10 flex flex-col gap-4">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-xl font-bold text-slate-900">{statement.name || statement.type}</h1>
                            ...
                            <div className="flex items-center gap-4 mt-2">
                                <div className="flex items-center gap-1.5 text-sm text-slate-600 font-medium">
                                    ...
                                    <span>Period: <span className="text-slate-900">{statement.period || `${formatDate(statement.startDate)} – ${formatDate(statement.endDate)}`}</span></span>
                                </div>
                    ...
                    <div className="flex gap-3">
                        {statement.status === 'Locked' ? (
                            <button
                                onClick={() => onLock && onLock({ ...statement, status: 'Draft' })}
                                                ...
                                                <td className="px-4 py-3 font-medium text-slate-700">
                                                    {isDraft ? (
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span className="font-medium text-slate-700">{line.raw}</span>
                                                            {line.status === 'unmapped' && (
                                                                <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold uppercase w-fit">
                                    ...
                                    {selectedLine.section && (
                                        <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    const val = e.target.value;
                                                    let category = 'Operating Expense'; // Default

                                                    // Income Statement Logic
                                                    if (['Sales / Revenue'].includes(val)) category = 'Revenue';
                                                    else if (val.includes('Cost of Goods Sold')) category = 'COGS';
                                                    else if (['Depreciation / Amortization (Non-Cash)'].includes(val)) category = 'Operating Expense'; // Handled via flag usually, but cat is OpEx
                                                    else if (['Interest Expense'].includes(val)) category = 'Operating Expense';

                                                    // Balance Sheet Logic
                                                    if (['Cash & Equivalents', 'Accounts Receivable', 'Inventory', 'Other Current Assets', 'Fixed Assets (Prop/Plant/Equip)', 'Intangible Assets'].includes(val)) category = 'Asset';
                                                    else if (['Accounts Payable', 'Credit Cards / Line of Credit', 'Current Portion of LTD', 'Long Term Debt (LTD)'].includes(val)) category = 'Liability';
                                                    else if (['Owner Equity / Retained Earnings'].includes(val)) category = 'Equity';

                                                    // Auto-set flags based on selection
                                                    const updates = { master: val, category };
                                                    if (val === 'Depreciation / Amortization (Non-Cash)') updates.isNonCash = true;
                                                    if (val === 'Interest Expense') updates.isInterest = true;
                                                    if (val === 'Owner Compensation') updates.isOwnerComp = true;

                                                    handleUpdateMapping(updates);
                                                }}
                                            >
                                                <option value="">-- Select Master Category --</option>
                                                {selectedLine.section === 'Income Statement' && (
                                                    <>
                                                        <optgroup label="Revenue & COGS">
                                                            <option>Sales / Revenue</option>
                                                            <option>Cost of Goods Sold (Materials)</option>
                                                            <option>Cost of Goods Sold (Labor)</option>
                                                        </optgroup>
                                                        <optgroup label="Expenses">
                                                            <option>Advertising / Marketing</option>
                                                            <option>Auto & Vehicle</option>
                                                            <option>Insurance</option>
                                                            <option>Interest Expense</option>
                                                            <option>Professional Services (Legal/CPA)</option>
                                                            <option>Rent / Lease</option>
                                                            <option>Repairs & Maintenance</option>
                                                            <option>Salaries & Wages (Non-Owner)</option>
                                                            <option>Owner Compensation</option>
                                                            <option>Supplies</option>
                                                            <option>Taxes & Licenses</option>
                                                            <option>Travel / Meals</option>
                                                            <option>Utilities</option>
                                                            <option>Depreciation / Amortization (Non-Cash)</option>
                                                            <option>Other Expense</option>
                                                        </optgroup>
                                                    </>
                                                )}
                                                {selectedLine.section === 'Balance Sheet' && (
                                                    <>
                                                        <optgroup label="Assets">
                                                            ...
                                                            <option>Fixed Assets (Prop/Plant/Equip)</option>
                                                            <option>Intangible Assets</option>
                                                        </optgroup>
                                                        <optgroup label="Liabilities & Equity">
                                                            <option>Accounts Payable</option>
                                                            <option>Credit Cards / Line of Credit</option>
                                                            <option>Current Portion of LTD</option>
                                                            <option>Long Term Debt (LTD)</option>
                                                            <option>Owner Equity / Retained Earnings</option>
                                                        </optgroup>
                                                    </>
                                                )}
                                            </select>
...
export default FinancialMappingModal;

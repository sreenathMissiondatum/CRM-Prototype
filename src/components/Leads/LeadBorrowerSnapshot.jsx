...
import { Building2, MapPin, Phone, Mail, User, CreditCard, BadgeCheck, Globe, Crown, Eye, EyeOff, Copy, Lock, Shield, MessageSquare, Send } from 'lucide-react';
import AvatarWithPresence from '../Shared/AvatarWithPresence';
import ChatContextPanel from '../Shared/ChatContextPanel';
...
const LeadBorrowerSnapshot = ({ data, contact }) => {
    ...
    // Chat / Message Logic
    const isOnlineOrBusy = ['online', 'busy'].includes(contact.status);
    const isDndOrOffline = !contact.status || ['offline', 'dnd'].includes(contact.status);

    const handleChatAction = () => {
        if (isOnlineOrBusy) {
            setIsChatOpen(true);
        } else {
            // Offline/DND Fallback
            setToast(`Message sent to ${contact.name}. It will be logged in the timeline.`);
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                        ...
                        <div className="flex-1 md:max-w-md">
                            ...
                            <div className="flex items-start gap-4">
                                <AvatarWithPresence
                                    src={contact.avatar}
                                    initials={contact.name ? contact.name.slice(0, 1) : null}
                                    name={contact.name}
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-2 text-sm text-slate-600 hover:text-blue-600 transition-colors cursor-pointer group">
                                            <Mail size={14} className="text-slate-400 group-hover:text-blue-500 shrink-0" />
                                            <span className="truncate">{contact.email}</span>
                                        </div>
                                    </div>

                                    {/* Action Row: Status / Chat */}
                                    <div className="mt-4 pt-3 border-t border-slate-50 flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-1.5 cursor-help" title="Estimated Credit Score">
                                        ...
                                        <button
                                            ...
                                            `}
                                            title={isOnlineOrBusy
                                                ? (contact.status === 'busy' ? "User is busy — responses may be delayed" : "Start chat")
                                                : "User is offline/DND - Send Message"
                                            }
                                        >
                                            {isOnlineOrBusy ? <MessageSquare size={14} /> : <Send size={14} />}
                                            {isOnlineOrBusy ? 'Chat' : 'Message'}
                                        </button>
                                    </div>
                                </div>
                            </div>

    ...
    const ProgramTile = ({ prog }) => {
        ...
        if (prog.status === 'Draft' && canManage) {
            menuActions.push({
                label: 'Edit Program',
                icon: Edit3,
                onClick: () => setViewDetailProgram(prog) // For MVP, Edit opens Detail
            });
            menuActions.push({
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 group relative">
                <div className="p-6">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div className="pr-8">
                            <h3 className="text-lg font-bold text-slate-900 leading-tight mb-1 group-hover:text-blue-600 transition-colors cursor-pointer" onClick={() => setViewDetailProgram(prog)}>
                                {prog.name}
                            </h3>
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${prog.authority === 'SBA' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>
                            {prog.authority}
                        </span>
